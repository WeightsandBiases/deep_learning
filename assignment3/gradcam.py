...
int_grads = GuidedGradCam(model, model.features[3])
attr_ig = compute_attributions(int_grads, X_tensor, target=y_tensor)
visualize_attr_maps('visualization/guided_gradcam_captum.png', X, y, class_names, [attr_ig], ['GuidedGradCam'])

# Computing Guided BackProp
int_grads = GuidedBackprop(model)
attr_ig = compute_attributions(int_grads, X_tensor, target=y_tensor)
visualize_attr_maps('visualization/guided_backprop_captum.png', X, y, class_names, [attr_ig], ['GuidedBackProp'])
##############################################################################
#                             END OF YOUR CODE                               #
##############################################################################
...
visualize_attr_maps('visualization/layer_activations_captum.png', X, y, class_names, [layer_act_attr_sum], ['Layer Activations'])
...
layer_gradcam = LayerGradCam(model.forward, layer)
...
visualize_attr_maps('visualization/layer_gradcam_captum.png', X, y, class_names, [layer_gradcam_attr_sum], ['Layer GradCam'])

layer_conductance = LayerConductance(model.forward, layer)
layer_conductance_attr = compute_attributions(layer_conductance, X_tensor, target=y_tensor)
layer_gradcam_attr_sum = layer_conductance_attr.mean(axis=1, keepdim=True)
visualize_attr_maps('visualization/layer_cond_captum.png', X, y, class_names, [layer_gradcam_attr_sum], ['Layer Conductance'])
