...
class CustomReLU(TorchFunc):
    ...
    def backward(self, y):
        ...
        ##############################################################################
        deconv = torch.addcmul(torch.zeros(y.size()), y, (y > 0).type_as(y))
        guided_backprop = torch.addcmul(torch.zeros(y.size()), deconv, (self.saved_tensors[0] > 0).type_as(self.saved_tensors[0]))
...
class GradCam:
    def guided_backprop(self, X_tensor, y_tensor, gc_model):
        ...
        ##############################################################################
        y_pred = gc_model(X_tensor)
        y_pred.backward(torch.ones_like(y_pred))
        return X_tensor.grad.permute(0, 2, 3, 1).detach().numpy()
        ##############################################################################
        #                             END OF YOUR CODE                               #
        ##############################################################################
    ...
    def grad_cam(self, X_tensor, y_tensor, gc_model):
        ...
        y_pred = gc_model(X_tensor)
        y_pred.backward(torch.ones_like(y_pred))
        cam = torch.sum((self.activation_value * self.gradient_value), dim=1)
        relu = torch.nn.ReLU()
        cam = relu(cam).detach().numpy()
        ##############################################################################
        #                             END OF YOUR CODE                               #
        ##############################################################################
